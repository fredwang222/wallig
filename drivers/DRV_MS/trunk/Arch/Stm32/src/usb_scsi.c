...
void SCSI_ReadFormatCapacity_Cmd(uint8_t lun)
{
  ...
  ReadFormatCapacity_Data[4] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Count >> 24);
  ReadFormatCapacity_Data[5] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Count >> 16);
  ReadFormatCapacity_Data[6] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Count >>  8);
  ReadFormatCapacity_Data[7] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Count);

  ReadFormatCapacity_Data[9] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Size >>  16);
  ReadFormatCapacity_Data[10] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Size >>  8);
  ReadFormatCapacity_Data[11] = (uint8_t)(DRV_MS_MemDevice[lun].Block_Size);
...
bool SCSI_Address_Management(uint8_t lun , uint8_t Cmd , uint32_t LBA , uint32_t BlockNbr)
{
  ...
  if ((LBA + BlockNbr) > DRV_MS_MemDevice[lun].Block_Count )
  {
    if (Cmd == SCSI_WRITE10)
    {
      Bot_Abort(BOTH_DIR);
  ...
  if (CBW.dDataLength != BlockNbr * DRV_MS_MemDevice[lun].Block_Size)
  {
    if (Cmd == SCSI_WRITE10)
    {
      Bot_Abort(BOTH_DIR);
