...
void Read_Memory(uint8_t lun, uint32_t Memory_Offset, uint32_t Transfer_Length)
{
  ...
  {
    Offset = Memory_Offset * DRV_MS_MemDevice[lun].Block_Size;
    Length = Transfer_Length * DRV_MS_MemDevice[lun].Block_Size;
  ...
  if (TransferState == TXFR_ONGOING )
  {
    if (!Block_Read_count)
    {
      /*MAL_Read(lun ,
   	  ...
   	  DRV_MS_MemDevice[lun].tIOAccess.Read (lun ,
											Offset ,
											Data_Buffer,
											DRV_MS_MemDevice[lun].Block_Size);
      USB_SIL_Write(EP1_IN, (uint8_t *)Data_Buffer, BULK_MAX_PACKET_SIZE);

      Block_Read_count = DRV_MS_MemDevice[lun].Block_Size - BULK_MAX_PACKET_SIZE;
...
void Write_Memory (uint8_t lun, uint32_t Memory_Offset, uint32_t Transfer_Length)
{
  ...
  if (TransferState == TXFR_IDLE )
  {
    W_Offset = Memory_Offset * DRV_MS_MemDevice[lun].Block_Size;
    W_Length = Transfer_Length * DRV_MS_MemDevice[lun].Block_Size;
    TransferState = TXFR_ONGOING;
  }
  ...
  if (TransferState == TXFR_ONGOING )
  {
    ...
    if (!(W_Length % DRV_MS_MemDevice[lun].Block_Size))
    {
      Counter = 0;
      /*MAL_Write(lun ,
                W_Offset - DRV_MS_MemDevice[lun].Block_Size,
                Data_Buffer,
                DRV_MS_MemDevice[lun].Block_Size);*/
   	  DRV_MS_MemDevice[lun].tIOAccess.Write(lun ,
                  W_Offset - DRV_MS_MemDevice[lun].Block_Size,
                  Data_Buffer,
                  DRV_MS_MemDevice[lun].Block_Size);
    }
