...
echo 'LIB_INC += -I $(PROJECT_PATH)/Lib/Fifo/' >> Lib.mk
fi
#Mp3
if grep -q "LIB_MP3=y" $PRJ_CONFIG_FILE
then
echo "LIB_OBJ += Lib/Mp3/libmp3.a" >> Lib.mk
echo 'LIB_INC += -I $(PROJECT_PATH)/Lib/Mp3/' >> Lib.mk
fi
#Ntl
...
if grep -q "LIB_NTL=y" $PRJ_CONFIG_FILE
then
echo '	@(cd Ntl && $(MAKE) clean)' >> Makefile
fi
